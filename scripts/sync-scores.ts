...
function parseArgs() {
  ...
  const now = new Date();
  return {
    week: get("--week") ? Number(get("--week")) : currentNflWeek(now),
...
// Rough estimate only, used as a default when --week isn't passed. The
...
// not a source of truth — pass --week explicitly for anything precise.
function currentNflWeek(now: Date): number {
